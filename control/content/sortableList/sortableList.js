class SortableList {
    ...
    deleteItem(propertyName, propertyValue, divRow){
        this.items = this.items.filter(item=>(item[propertyName]!==propertyValue));
        divRow.remove();
    }
    ...
    changeToggle(item, isActive, divRow) {
        this.items = this.items.map((_item,idx)=>{
            if(item.id!==_item.id){
                return _item;
            }else{
